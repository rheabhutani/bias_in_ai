...
class Model(namedtuple('Model', 'pred label')):
    ...
    def calibration(self, other, fp_rate, fn_rate, mix_rates=None):
        if fn_rate == 0:
        ...
        calibration_self = Model(self_new_pred, self.label)
        ...
        calibration_other = Model(other_new_pred, other.label)

        if mix_rates is None:
            return calibration_self, calibration_other, (self_mix_rate, other_mix_rate)
        else:
            return calibration_self, calibration_other
...
if __name__ == '__main__':
    ...
    ```
    python calibration.py <path_to_model_predictions.csv> <cost_constraint>
    ```
    ...
    Try the following experiments for testing:
    ```
    python calibration.py data/income.csv fnr
    python calibration.py data/health.csv weighted
    python calibration.py data/criminal_recidivism.csv fpr
    ```
    """
    import pandas as pd
    import sys
    ...
    _, _, mix_rates = Model.calibration(group_0_val_model, group_1_val_model, fp_rate, fn_rate)

    # Apply the mixing rates to the test models
    calibration_group_0_test_model, calibration_group_1_test_model = Model.calibration(group_0_test_model,
                                                                                             group_1_test_model,
                                                                                             fp_rate, fn_rate,
                                                                                             mix_rates)
    ...
    print('Equalized odds group 0 model:\n%s\n' % repr(calibration_group_0_test_model))
    print('Equalized odds group 1 model:\n%s\n' % repr(calibration_group_1_test_model))
